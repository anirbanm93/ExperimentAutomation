...
class SpectrumTest:
    ...
    def collect_spectra(self) -> Tuple[float, NDArray[np.float64]]:
        ...
        data = np.array(data, dtype=np.float64)  # shape: (len(pwr_in), len(f_in), trace_pts)
    ...
    def _write_metadata_to_h5(self, f: h5py.File) -> None:
        ...
        - Instrument configurations are stored in separate groups:
          `rfsa_settings` and `rfgen_settings`.
...
if __name__ == "__main__":
    ...
    input_pwr = np.array([-30, -25]) * dbm

    # Input frequency variation
    input_freq = np.linspace(2000, 2150 + 200, 3501) * megahz
    ...
    if 1:
        spectrumtest = SpectrumTest(pwr_in=input_pwr,
                                    f_in=input_freq,
        ...
        spectrumtest.repeat_collect_and_save_spectra(
            ...
            savefilename=f"2025114_spectrum_test_yig_s18_port1_to_2",
            num_iters=20,
            delay=2 * secs)
